...
public class ViewAppointment extends BaseEntity {
    ...
    // @JsonFormat注解用于指定日期时间的格式化方式，pattern属性定义了日期时间的格式，timezone属性指定了时区。
    // 也可以在application.yml中全局配置Jackson的日期时间格式化方式。
    @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss", timezone = "GMT+8")  // 向前端传递时间时的格式化
